do_sampling_stratified_timeseries<-function(sampDes, sampOpt, dataset, sampId){

	# resamples stations from strata (time series)
	...
	# 2019-05-01 # 1st of May?! seriously?! this is shameful and not to repeat - long live May day [https://en.wikipedia.org/wiki/May_Day]
...
# check on sample sizes
	if(sampOpt$replacement==FALSE)
		{
		#if(any(sampOpt$samp_sizes>nrow(dataset)) & sampOpt$sample_all_available==FALSE) {stop (cat("\n ATT: samp_sizes >  population size with replace = FALSE AND sample_all_available = FALSE\n\n"))}
	...
	for (j in 4:ncol(sampOpt$samp_sizes))
	{
